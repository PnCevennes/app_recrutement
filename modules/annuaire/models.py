...
class Entite(db.Model):
    ...
    def relations(self):
        ...
        relations = Entite.query.filter(Entite.id.in_([r.id_enfant for r in rels])).all()
        return [{'id':rel.id, 'label': rel.nom} for rel in relations]
    ...
    def parents(self):
        ...
        return [{'id':rel.id, 'label': rel.nom} for rel in parents]
